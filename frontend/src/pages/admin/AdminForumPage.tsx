      ...
      {tab === 4 && (
        <Card sx={{ bgcolor: forumPalette.bgCard, border: `1px solid ${forumPalette.border}` }}>
          <CardContent>
            ...
            <Typography variant="body2" mb={2} sx={{ color: forumPalette.textSecondary }}>
              Moderadores de foro por departamento. Para asignar el rol MODERATOR a un usuario usá la
              pestaña «Usuarios» del panel de administración (roles del core).
            </Typography>
            {moderators.length === 0 ? (
              <Alert severity="info">No hay moderadores de foro. Asigná el rol MODERATOR desde «Usuarios».</Alert>
            ) : (
              <Stack spacing={1}>
                {moderators.map((m) => (
