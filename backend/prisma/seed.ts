...
async function seedRbac() {
  // Roles de fábrica (isSystem, no se borran)
  ...
  // Asignaciones: ADMIN → todos los permisos y menús; SELLER → seller + públicos; CUSTOMER → públicos; MODERATOR → foro + públicos + admin.forum
  const permissionCodes: Record<string, string[]> = {
    ...
    CUSTOMER: ['forum.post'],
    MODERATOR: ['forum.post', 'forum.moderate', 'forum.geo.manage'],
  };
  const menuCodes: Record<string, string[]> = {
    ...
    MODERATOR: [...RBAC_MENUS.filter((m) => m.module === 'public').map((m) => m.code), 'admin.forum'],
  };
